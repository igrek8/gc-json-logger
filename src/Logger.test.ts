...
describe('Logger', () => {
  ...
  it('prints logger name', async () => {
    const logger = new Logger('context');
    Logger.runWith(logger, () => {
      expect(Logger.getName()).toBe('context');
    });
  });

  it('retuns a return value from a callback', async () => {
    const logger = new Logger('context');
    await expect(Logger.runWith(logger, () => Promise.resolve('test'))).resolves.toBe('test');
  });
